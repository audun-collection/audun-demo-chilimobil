...
/**
 * Marketing-demo header. Pill nav with the single customer-facing tab
 * (Besparelse). All operator chrome from the production app is
 * intentionally absent — this repo only serves /demo/besparelse.
 */
    ...
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50 flex justify-center px-6 pt-5">
      <div className="nav-pill pointer-events-auto flex w-full max-w-6xl items-center justify-between gap-8 rounded-full py-2.5 pl-5 pr-3.5">
        <div className="flex items-center gap-8">
          <Link href="/demo/besparelse" className="flex items-center gap-2.5 text-ink-900">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/solvva-logo.svg" alt="" width={22} height={22} />
            <span className="wordmark">Audun</span>
          </Link>
          <nav className="flex items-center gap-0.5 text-sm">
            <NavLink href="/demo/besparelse">Besparelse</NavLink>
          </nav>
        </div>
